...
internal class OverTest {
    ...
    fun `should not mark over completed when it has less than 6 legal deliveries`() {
        val noWicketBall = NoWicketBall(NormalRuns(2))
        val wicketBall = WicketBall(wicket = Bowled(0))
        val wideBall = WideBall(Byes(2))
    ...
    fun `should give total wickets in the over`() {
        ...
        assertEquals(
            Over(0, listOf(wicketBall, NoWicketBall(NormalRuns(1)))).totalWickets(),
            1
        )
    ...
    fun `should give total number of runs`() {
        ...
        assertEquals(
            Over(
                0, listOf(
                    noWicketBall, WideBall(), WicketBall(
                        run = NormalRuns(3), wicket = Bowled(
                            playerId = 0
                        )
                    )
                )
